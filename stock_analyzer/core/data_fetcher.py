    ...
    def get_stock_data(self, symbol: str, period: str = "1y", interval: str = "1d") -> pd.DataFrame:
        ...
        try:
            ...
            if "Error Message" in data or "Note" in data or "Information" in data:
                err = data.get("Note", data.get("Error Message", data.get("Information", "Unknown")))
    ...
    def get_stock_data(
        ...
        try:
            ...
            try:
                ...
                if not data.empty:
                    data.attrs["data_source"] = "demo/simulated"
                    logger.warning(f"使用模拟数据: {symbol} - 真实数据获取失败，所有技术指标基于模拟价格")
                    return data
            except ImportError:
                pass
